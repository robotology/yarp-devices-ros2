...
bool RgbdSensor_nws_ros2_ParamsParser::getParamValue(const std::string& paramName, std::string& paramValue) const
{
    if (paramName =="period")
    {
    ...
    if (paramName =="force_info_synch")
    {
        paramValue = std::to_string(m_force_info_synch);
        return true;
    }
    if (paramName =="depth_frame_id")
    {
        paramValue = m_depth_frame_id;
        return true;
    }
...
std::string RgbdSensor_nws_ros2_ParamsParser::getConfiguration() const
{
